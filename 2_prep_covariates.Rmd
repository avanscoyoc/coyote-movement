...
lc <- raster(here("data_covariates", "covariates_from_GEE", "HREC_landcover_extended.tif"))
...
dummy_data <- data.frame(used = rbinom(30, 1, 0.4),
                         vegetation = factor(rep(c("Shrub", "Woodland", "Grassland"), each = row.num), 
...
hunter <- tm_shape(hrec_hunter_risk) + 
            ...
            tm_layout(legend.show = F, panel.show=T, panel.labels="HREC Hunter Risk")
```
