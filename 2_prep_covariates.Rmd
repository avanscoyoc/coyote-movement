...
#convert vegetation raster to categorical values (For lions)
type <- data.frame(ID=1:3, cover=c("Shrubland", "Woodland", "Grassland"))
levels(veg) <- type
```
...
dummy_data <- data.frame(used = rbinom(30, 1, 0.4),
                         vegetation = factor(rep(c("Shrubland", "Woodland", "Grassland"), each = 10), 
                                              levels = c("Shrubland", "Woodland", "Grassland")),
...
road_sc <- sdmvspecies::rescale(road_dist)
tri_sc <- sdmvspecies::rescale(tri)
...
veg_table <- unique(data.frame(vegetation = c(1,2,3),
                               vegetation_class = as.factor(c("Shrubland","Woodland","Grassland"))))
...
hunter <- hunt_rsf %>% setValues(hunter_discrete)

#create mask for where hunters won't hunt (150m of development)
dev_mask <- raster::buffer(dev, width = 150)
hunter_risk <- mask(hunter, dev_mask, inverse=TRUE) #remove dev from hunter risk

# replacing NA's by zero
hunter_risk[is.na(hunter_risk[])] <- 0 
...
# hrec_crop <- crop(hunter_risk, extent(hopland))
# hrec_hunter_risk <- mask(hrec_crop, st_zm(hopland))
# 
hunter <- tm_shape(hunt_rsf) + tm_raster(style = "quantile") +
  tm_layout(aes.palette = list(seq = "YlOrRd"),
            legend.show = T, panel.show=T,
            panel.labels="Lethal Removal Risk", panel.label.size = 1,
            legend.title.color = "white", legend.outside = T,
...
dummy_data2 <- data.frame(used = rbinom(30, 1, 0.4),
                         vegetation = factor(rep(c("Shrubland", "Woodland", "Grassland"), each = 10), 
                                              levels = c("Shrubland", "Woodland", "Grassland")),
                         inv_crop_dist = c(runif(30, min = 0, max = 2500))) 

#Recreate formula from Gaynor et. al 2021 and specify interactions:
mod2 <- glm(formula = used ~ vegetation + inv_crop_dist, 
...
crop_sc <- sdmvspecies::rescale(crop_dist)
crop_inverse = crop_sc
values(crop_inverse) <- 1 - values(crop_sc) 
plot(crop_inverse)

#stack layers
lion_stack <- brick(veg, crop_inverse) 
names(lion_stack) <- c("vegetation", "inv_crop_dist")
...
```{r}
lion <- exp(lion_risk)/max(values(exp(lion_risk)), na.rm = TRUE)

#remove dev from lion risk
lion_risk <- mask(lion, dev_mask, inverse=TRUE)

# replacing NA's by zero
lion_risk[is.na(lion_risk[])] <- 0 
...
```{r}
# #crop to hopland boundary to see if its accurate
# lion_crop <- crop(lion_risk, extent(hopland))
...
#           tm_layout(legend.show = T, panel.show=T, panel.labels="HREC Lion Risk")
```
...
names(envt) <- c("Elevation", "Ruggedness", "Vegetation Type", "Development Dist.",
                 "Road Dist.","Agriculture Dist", "Lethal Removal Risk", "Mountain Lion Risk")
...
tmlayer <- function(layer,title, color){
  tm_shape(layer) + 
    tm_raster(style = "cont", palette = "seq") +
  tm_layout(aes.palette = list(seq = color)) +
    tm_layout(legend.show = T, panel.show=T, panel.labels=title, panel.label.size = 1,
              legend.title.color = "white", legend.outside = T, title.position=c("center","bottom")) 
}

tmap_arrange(
  tmlayer(envt[[2]], "Ruggedness", "Spectral"),
  tmlayer(envt[[8]], "Mountain Lion Risk", "YlOrRd"),
  tmlayer(envt[[4]], "Development Dist.", "-YlOrRd"),
  tmlayer(envt[[5]], "Road Dist.", "-YlOrRd"), 
  tmlayer(envt[[6]], "Agriculture Dist.", "-YlGn"), 
  hunter, 
  tmlayer(envt[[3]], "Vegetation type", "Dark2"), ncol = 3) %>% 
  tmap_save('figs/Figure_S1.png')
```
