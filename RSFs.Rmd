...
cand.models <- c(combined, lion, hunter, humans, livestock, crop, habitat)
names(cand.models) <- c("Full model", "Lion risk", "Hunter risk", "Human presence", "Livestock distance", "Cropland distance", "Habitat distance")
indv_mods <- data.frame(aictab(cand.models)) %>% 
  dplyr::select(1,"LL", "AICc","Delta_AICc") %>% 
  rename(Model = Modnames, Log_Likelihood = LL)
tab_df(indv_mods)
...
pp1 <- plot_model(combined,type="pred")
pp2 <- plot_model(rest_mod,type="pred")
plot_grid(c(pp1,pp2))
...
jtools::plot_summs(combined, rest_mod, forage_mod, travel_mod, scale = TRUE, 
           ...
           coefs = c("Lion risk"="lion.scaled", 
                     ...
                     "Distance to Development"="humans.scaled", 
                     "Distance to Livestock"="livestock.scaled",
                     "Distance to Cropland"="crop.scaled", 
                     "Distance to Habitat"="habitat.scaled"), 
           colors = c("black","#E7B800", "#00afbb", "seagreen")) 
```
